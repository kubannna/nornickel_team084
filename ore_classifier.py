...
def train(args):
    ...
    # --- transfer learning: тёплый старт с существующего чекпоинта ---
    if getattr(args, "init_from", None):
        _ck = torch.load(args.init_from, map_location=device)
        _sd = _ck["model"] if isinstance(_ck, dict) and "model" in _ck else _ck
        _old_n = _ck.get("num_classes") if isinstance(_ck, dict) else None
        if _old_n is not None and _old_n != num_classes:
            _sd = {k: v for k, v in _sd.items() if not k.startswith("fc.")}
            print(f"[transfer] число классов {_old_n}->{num_classes}: fc-голова учится заново")
        missing, unexpected = model.load_state_dict(_sd, strict=False)
        print(f"[transfer] init-from {args.init_from} "
              f"(missing={len(missing)}, unexpected={len(unexpected)})")

    # --- freeze backbone: дообучаем только голову fc ---
    if getattr(args, "freeze_backbone", False):
        for name, p in model.named_parameters():
            p.requires_grad = name.startswith("fc.")
        print("[transfer] backbone заморожен — учим только fc-голову")
...
def parse_args():
    ...
    ap.add_argument("--init-from", default=None,
                    help="дообучение: тёплый старт с существующего .pt (transfer learning)")
    ap.add_argument("--freeze-backbone", action="store_true",
                    help="заморозить backbone (учить только fc-голову)")
