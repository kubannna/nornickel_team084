...
canvas_max_side = st.sidebar.slider("Размер холста коррекции (px)", 512, 1400, 900, 100)
...
def correction_block(row, idx):
    ...
    if _HAS_CANVAS:
        layer = st.radio("Слой", ["Сульфиды", "Тальк"], key=f"layer_{idx}", horizontal=True)
        mode = st.radio("Действие", ["Добавить", "Стереть"], key=f"mode_{idx}", horizontal=True)
        stroke = st.slider("Толщина кисти", 2, 60, 16, key=f"stroke_{idx}")

        scale = min(1.0, canvas_max_side / float(max(H, W)))
        disp_w, disp_h = max(1, int(W * scale)), max(1, int(H * scale))

        _base = np.asarray(Image.open(row["image"]).convert("RGB").resize((disp_w, disp_h))).copy()
        _sulf_s = cv2.resize(sulf, (disp_w, disp_h), interpolation=cv2.INTER_NEAREST)
        _talc_s = cv2.resize(talc, (disp_w, disp_h), interpolation=cv2.INTER_NEAREST)
        _ov = _base.copy()
        _ov[_sulf_s > 127] = (0, 255, 0)
        _ov[_talc_s > 127] = (255, 0, 255)
        _blended = cv2.addWeighted(_base, 0.55, _ov, 0.45, 0)
        bg = Image.fromarray(_blended.astype("uint8"))
        stroke_color = "#00ff00" if layer == "Сульфиды" else "#ff00ff"
        ...
        st.caption("Подложка: зелёный — текущие сульфиды, розовый — текущий тальк. "
                   "Кисть рисует цветом выбранного слоя.")

        if st.button("Применить правки (с холста)", key=f"apply_{idx}"):
            has_paint = (canvas is not None and canvas.image_data is not None
                         and bool((np.asarray(canvas.image_data)[..., 3] > 0).any()))
            if has_paint:
                painted_small = (np.asarray(canvas.image_data)[..., 3] > 0).astype(np.uint8) * 255
                painted = cv2.resize(painted_small, (W, H), interpolation=cv2.INTER_NEAREST)
                target = sulf if layer == "Сульфиды" else talc
                if mode == "Добавить":
                    target = np.where(painted > 0, 255, target).astype(np.uint8)
                else:
                    target = np.where(painted > 0, 0, target).astype(np.uint8)
                if layer == "Сульфиды":
                    row["sulfide_mask"] = target
                else:
                    row["talc_mask"] = target
                rc = _reclassify()
                st.success(f"Пересчитано: {rc['class']} (тальк {rc['talc_percent']}%, "
                           f"обычные {rc['normal_percent']}%, тонкие {rc['fine_percent']}%)")
            else:
                st.warning("Нарисуйте правки на холсте перед применением.")
    else:
        st.info("Холст недоступен (компонент streamlit-drawable-canvas не загрузился). "
                "Используйте загрузку исправленной маски ниже — она работает всегда.")
