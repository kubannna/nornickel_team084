...
def train(args):
    ...
    # --- freeze энкодера: дообучаем только decoder+head ---
    if getattr(args, "freeze_encoder", False):
        n_frozen = 0
        for p in model.encoder.parameters():
            p.requires_grad = False
            n_frozen += 1
        print(f"[transfer] энкодер заморожен ({n_frozen} тензоров) — учим только decoder+head")
...
def parse_args():
    ...
    ap.add_argument("--freeze-encoder", action="store_true",
                    help="заморозить энкодер (учить только decoder+head)")
